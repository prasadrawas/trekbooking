...
function previewScheduleDates(scheduleType: ScheduleType, form: ScheduleFormState): string[] {
  const dates: string[] = [];
...
function Step6Schedule({
  ...
  const [form, setFormState] = useState<ScheduleFormState>({
    ...INITIAL_SCHEDULE_FORM,
    adultPrice: adultPrice || INITIAL_SCHEDULE_FORM.adultPrice,
    childPrice: childPrice || INITIAL_SCHEDULE_FORM.childPrice,
  });
  ...
  const setF = <K extends keyof ScheduleFormState>(k: K, v: ScheduleFormState[K]) =>
    setFormState((prev) => ({ ...prev, [k]: v }));

  const preview = useMemo(() => previewScheduleDates(scheduleType, form), [scheduleType, form]);
  ...
  // Fetch events when trekId becomes available
  useState(() => { fetchEvents(); });

  async function handleAddSchedule() {
    if (!trekId) {
      setSchedError("Trek must be saved before scheduling dates. Please go back and save first.");
      return;
    }
    setSubmitting(true);
    setSchedError("");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Add scheduled dates for this trek. Trekkers can then book slots on these dates.
        </p>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input type="date" value={form.endDateRange} onChange={(e) => setF("endDateRange", e.target.value)} />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input
              ...
              onChange={(e) => setF("dayOfMonth", Math.min(28, Math.max(1, Number(e.target.value))))}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            <Input
              ...
              onChange={(e) => setF("customDateInput", e.target.value)}
              className="flex-1"
            />
      ...
      <div className="border-t border-slate-100 pt-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input type="time" value={form.reportingTime} onChange={(e) => setF("reportingTime", e.target.value)} />
          </div>
          <div>
      ...
      {!trekId && (
        <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800">
          Save the trek first (go back to any step and click Save &amp; Next) to enable scheduling.
        </div>
      )}
      ...
      <Button
        ...
        disabled={submitting || preview.length === 0 || !trekId}
                ...
                <div key={ev.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      {ev.reportingTime} · ₹{ev.adultPrice} · {ev.bookedSeats}/{ev.totalSeats} seats
                    </p>
                  </div>
                  <span className={cn(
      ...
      <p className="text-xs text-slate-400 text-center">
        You can also skip this step and schedule dates later from the trek dashboard.
      </p>
    </div>
  );
}
...
export default function NewTrekPage({ isEdit = false }: { isEdit?: boolean }) {
  ...
  const goNext = async () => {
    // Step 6 (Schedule) doesn't need to save trek data, just advance
    if (step === 6) {
      setDirection(1);
      setStep(7);
      return;
